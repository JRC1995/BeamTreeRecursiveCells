...
class BeamGumbelTreeCell_transparent(nn.Module):
    def __init__(self, config):
        super(BeamGumbelTreeCell_transparent, self).__init__()
        self.config = config
    ...
    def select_composition(self, old_state, new_state, mask, accu_scores, beam_mask, past_paths):
        ...
        if (B * topk) > self.beam_size:
            ...
            past_paths = T.matmul(beam_select_mask, past_paths)
            assert past_paths.size() == (N, B2, SP + S)
        else:
            B2 = B * topk
    ...
    def forward(self, input, input_mask):
        ...
        return {"sequence": sequence, "global_state": global_state,
                "input_mask": input_mask, "aux_loss": aux_loss,
