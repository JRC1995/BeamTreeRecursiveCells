...
from models.topk_module import TopK_custom1
...
class DiffSortBeamTreeCell(nn.Module):
    def __init__(self, config):
        super(DiffSortBeamTreeCell, self).__init__()
        self.config = config
        ...
        self.diffsort = TopK_custom1(k=self.beam_size)
    ...
    def select_composition(self, old_state, new_state, mask, accu_scores, beam_mask):
        ...
        if (B * topk) > self.beam_size:
            ...
            permute_mask = self.diffsort(accu_scores_)[0]
            assert permute_mask.size() == (N, B * topk, B2)
            permute_mask = permute_mask.permute(0, 2, 1).contiguous()
            assert permute_mask.size() == (N, B2, B * topk)
