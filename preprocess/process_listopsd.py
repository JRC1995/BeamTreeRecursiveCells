...
train_path = Path('../data/listops/train_d6s.tsv')
dev_path = {}
dev_path["normal"] = Path('../data/listops/dev_d7s.tsv')
test_path = {}
test_path["dg8"] = Path('../data/listops/test_dg8s.tsv')
test_path["LRA"] = Path('../data/listops/basic_test.tsv')


for key in test_keys:
    if key != "dg8" and key != "LRA":
        test_path[key] = Path('../data/listops/test_{}.tsv'.format(key))

Path('../processed_data/listopsd/').mkdir(parents=True, exist_ok=True)

train_save_path = Path('../processed_data/listopsd/train.jsonl')
dev_save_path = {}
for key in dev_keys:
    dev_save_path[key] = Path('../processed_data/listopsd/dev_{}.jsonl'.format(key))
test_save_path = {}
for key in test_keys:
    test_save_path[key] = Path('../processed_data/listopsd/test_{}.jsonl'.format(key))

metadata_save_path = fspath(Path("../processed_data/listopsd/metadata.pkl"))
